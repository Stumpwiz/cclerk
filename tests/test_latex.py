...
\\end{document}
"""
        template_path = os.path.join(report_dir, "lfr_template.tex")
        with open(template_path, "w", encoding="utf-8") as f:
            f.write(template_content)
        ...
        try:
            ...
            output_path = os.path.join(report_dir, "test_long_form_roster.tex")
...
\\end{document}
"""
        latex_path = os.path.join(report_dir, "test_compilation.tex")
        pdf_path = os.path.join(report_dir, "test_compilation.pdf")
        ...
        try:
            ...
            result = subprocess.run(
                ["xelatex", "-interaction=nonstopmode", "-output-directory", report_dir, latex_path],
                cwd=report_dir,
                capture_output=True,
                text=True
            )
            ...
            for ext in [".tex", ".pdf", ".aux", ".log"]:
                try:
                    os.remove(os.path.join(report_dir, f"test_compilation{ext}"))
                except FileNotFoundError:
                    pass
...
"""
        ...
        try:
            ...
            for template_name in templates:
                template_path = os.path.join(report_dir, template_name)
                with open(template_path, "w", encoding="utf-8") as f:
                    f.write(template_content)
            ...
            for template_name in templates:
                try:
                    os.remove(os.path.join(report_dir, template_name))
                except FileNotFoundError:
                    pass
                
                output_name = f"test_{template_name}"
                try:
                    os.remove(os.path.join(report_dir, output_name))
                except FileNotFoundError:
                    pass
